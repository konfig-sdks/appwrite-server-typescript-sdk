...
import { readableStreamToString, AppwriteServerError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AppwriteServerError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AppwriteServerError(e, e.response?.data)
                }
                if (innerError instanceof AppwriteServerError) {
                    // Got "AppwriteServerError" from the above try block
                    throw innerError;
                }
